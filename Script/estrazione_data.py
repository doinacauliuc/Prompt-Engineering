...
def estrai_data_e_corpo(lines):
    """
    Estrae la data (prima riga che inizia con ~) e il corpo dell'articolo
    """
    data_pubblicazione = ""
    start_idx = None
    for idx, line in enumerate(lines):
        if line.strip().startswith("~"):
            data_pubblicazione = line.strip().lstrip("~").strip()
            start_idx = idx
            break
    ...
    # Corpo: tutte le righe DOPO la data (escludendo la riga data stessa)
    ...
    return data_pubblicazione, " ".join(corpo)
...
with open(output_pool_esempi, "w", newline="", encoding="utf-8-sig") as f_es, \
    ...
    # Aggiungo la colonna "data_pubblicazione"
    header = ["articolo", "data_pubblicazione", "localizzazione", "tempo", "grave", "moto", "utenti_deboli"]
    ...
    for idx, raw_article in enumerate(raw_articles):
        ...
        data_pubblicazione, article_text = estrai_data_e_corpo(lines)
        ...
        if article_text:
            row = [article_text, data_pubblicazione, 0, 0, 0, 0, 0]
            if idx < 200:
